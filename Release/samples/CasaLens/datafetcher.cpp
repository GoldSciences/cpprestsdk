...
pplx::task<web::json::value>	CasaLens::			handle_exception	(pplx::task<web::json::value>& t, const utility::string_t& field_name)			{
    try {
        ...
        error_json[field_name][json_key_error]				= web::json::value::string(utility::conversions::to_string_t(ex.what()));
...
// JSON result Format: {"events":["title":"event1","url":"http://event1","starttime":"ddmmyy",..]}
pplx::task<web::json::value>	CasaLens::			get_events			(const utility::string_t& postal_code)									{
    web::http::uri_builder									ub					(casalens_creds::url_events + postal_code);

    ub.append_query(casalens_creds::keyname_events, casalens_creds::key_events);
    ...
    return ev_client.request(web::http::methods::GET, event_url).then([](web::http::http_response resp) {
        ...
        if (!event_json[U("events")][U("event")].is_null()) {
            ...
            for(auto& iter : event_json[U("events")][U("event")].as_array()) {
				...
				key	= U("title"			);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("title"			)] = objectIterator->second;
				key	= U("url"			);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("url"			)] = objectIterator->second;
				key	= U("start_time"	);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("start_time"	)] = objectIterator->second;
				key	= U("description"	);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("description"	)] = objectIterator->second;
				key	= U("city_name"		);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("city_name"		)] = objectIterator->second;
				key	= U("venue_address"	);	objectIterator = event.as_object().find(key);	if (objectIterator == event.as_object().end()) { throw web::json::json_exception(key.c_str()); }	event_result_node[json_key_events][i][U("venue_address"	)] = web::json::value::string(objectIterator->second.as_string() + U(" ") + event_result_node[json_key_events][i][U("city_name")].as_string());
                if (i++ > num_events)
                    break;
            }
        }
        else { // Event data is null, we hit an error.
            event_result_node[json_key_events]					= web::json::value::object();
            event_result_node[json_key_events][json_key_error]	= event_json[U("events")][U("description")];
        }

        return event_result_node;
    }).then([=](pplx::task<web::json::value> t) {
        return handle_exception(t, json_key_events);
    });
}
...
pplx::task<web::json::value>	CasaLens::			get_weather			(const utility::string_t& postal_code)									{
    utility::string_t										weather_url			(casalens_creds::url_weather);
    ...
    return weather_client.request(web::http::methods::GET).then([](web::http::http_response resp) {
        ...
        weather_result_node[json_key_weather]				= web::json::value::object();
        ...
        return handle_exception(t, json_key_weather);
    });
}
...
pplx::task<web::json::value>	CasaLens::			get_pictures		(const utility::string_t& location, const utility::string_t& count)		{
    ...
    web::http::client::credentials							cred				(casalens_creds::keyname_images, casalens_creds::key_images);
    config.set_credentials(cred);
    utility::string_t										bing_url			(casalens_creds::url_images);
    ...
    return bing_client.request(web::http::methods::GET).then([](web::http::http_response resp) {
        ...
        image_result_node[json_key_images]					= web::json::value::array();
        ...
        for(auto& val : image_json[U("d")][U("results")].as_object()) {
            ...
            image_result_node[json_key_images][i]				= iMediaUrl->second;
            if (i++ > num_images)
                break;
        }
        return image_result_node;
    }).then([=](pplx::task<web::json::value> t) {
        return handle_exception(t, json_key_images);
    });
}
...
pplx::task<web::json::value>	CasaLens::			get_movies			(const utility::string_t& postal_code)									{
    web::http::uri_builder									ub_movie			(casalens_creds::url_movies);

    ub_movie.append_query(U("startDate"), get_date());
    ub_movie.append_query(U("zip"), postal_code);
    ub_movie.append_query(casalens_creds::keyname_movies, casalens_creds::key_movies);
    ...
    return tms_client.request(web::http::methods::GET).then([](web::http::http_response resp) {
        ...
        if (movie_json.size()) {
            ...
            movie_result_node[json_key_movies]					= std::move(temp);
        }
        else {
            movie_result_node[json_key_movies]					= web::json::value::object();
            movie_result_node[json_key_movies][json_key_error]	= web::json::value::string(U("Failed to fetch movie data"));
        }
        ...
        try {
            ...
            for(auto& iter : movie_result[json_key_movies].as_object()) {
            ...
            for(unsigned int i = 0; i < poster_tasks.size(); i++) {
                ...
                movie_result[json_key_movies][i][U("poster")]		= poster_url;
            }
        }
		catch(...) {}	// Ignore any failure in fetching movie posters. Just return the movie data to the client.

        return pplx::task_from_result(movie_result);
    }).then([=](pplx::task<web::json::value> t) {
        return handle_exception(t, json_key_movies);
    });
}
	...
	)						
{
    ...
    try {
        ...
        message.reply(web::http::status_codes::OK, resp_data).then([](pplx::task<void> t) { 
			handle_error(t); 
		});
    }
    catch(...) {
        message.reply(web::http::status_codes::InternalError).then([](pplx::task<void> t) { 
			handle_error(t); 
		});
    }
}
...
void							CasaLens::			get_data			(web::http::http_request message, const std::wstring& input_text)		{
    if (!is_number(utility::conversions::to_utf8string(input_text))) {
        std::wstring											bing_maps_url		(casalens_creds::url_bmaps);
        web::http::uri_builder									maps_builder;
        maps_builder.append_query(U("locality"), input_text);
        maps_builder.append_query(casalens_creds::keyname_bmaps, casalens_creds::key_bmaps);
        ...
        bing_client.request(web::http::methods::GET, s).then([=](web::http::http_response resp) {
            ...
            ub.append_path(lattitude + U(",") + longitude).append_query(casalens_creds::keyname_bmaps, casalens_creds::key_bmaps);
        ...
        web::http::client::http_client							client				(casalens_creds::url_gmaps);
