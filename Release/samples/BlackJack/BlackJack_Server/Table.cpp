...
void									DealerTable::Deal							()											{
    ...
    for (size_t player = m_currentPlayer+1; player < Players.size(); player++) {
        ...
        if ( Players[player].Hand.state == HR_Active ) {
            m_responses[player]->reply(web::http::status_codes::OK, BJPutResponse(ST_YourTurn, this->AsJSON()).AsJSON());
    ...
    for (size_t i = 1; i < Players.size(); i++)
        if ( i != m_currentPlayer ) {
            if ( m_responses[i] ) {
                m_responses[i]->reply(web::http::status_codes::OK, BJPutResponse(ST_Refresh, this->AsJSON()).AsJSON());
                m_responses[i].reset();
            }
            else
...
void									DealerTable::Hit							(web::http::http_request message)			{
    Card													card							= m_shoe.front(); m_shoe.pop();
    Players[m_currentPlayer].Hand.AddCard(card);

    if( Players[m_currentPlayer].Hand.state == HR_BlackJack 
	 || Players[m_currentPlayer].Hand.state == HR_Busted 
	 )
        Stay(message);
    else {
...
void									DealerTable::DoubleDown						(web::http::http_request message)			{
...
void									DealerTable::Wait							(web::http::http_request message)			{
    ...
    if ( playerIdx > 0 ) {
        ...
        if ( m_pendingrefresh[playerIdx] != ST_None ) {
            message.reply(web::http::status_codes::OK, BJPutResponse(m_pendingrefresh[playerIdx], this->AsJSON()).AsJSON());
            m_pendingrefresh[playerIdx]							= ST_None;
        }
        else
            m_responses[playerIdx]								= message_wrapper(new web::http::http_request(message));
    }
}

void									DealerTable::Bet							(web::http::http_request message)			{
    int														amount;
    utility::string_t										name;
    auto													query							= web::http::uri::split_query(web::http::uri::decode(message.relative_uri().query()));
    auto													itrAmount						= query.find(AMOUNT)
...
void									DealerTable::Insure							(web::http::http_request message)			{
    int														amount;
    utility::string_t										name;
...
void									DealerTable::Stay							(web::http::http_request message)			{
    ...
    message.reply(web::http::status_codes::OK, BJPutResponse(ST_Refresh, this->AsJSON()).AsJSON());

    NextPlayer(message);
}

void									DealerTable::NextPlayer						(web::http::http_request message)			{
...
void									DealerTable::PayUp							(size_t idx)								{
    Player													& player						= Players[idx];
		 if ( player.Hand.result == HR_PlayerWin		) { player.Balance += player.Hand.bet*2;	player.Hand.bet = 0.0;	}
    else if ( player.Hand.result == HR_ComputerWin		) {											player.Hand.bet = 0.0;	}
    else if ( player.Hand.result == HR_PlayerBlackJack	) { player.Balance += player.Hand.bet*2.5;	player.Hand.bet = 0.0;	}
    else if ( player.Hand.result == HR_Push				) { player.Balance += player.Hand.bet;		player.Hand.bet = 0.0;	}
    ...
    if ( m_responses[idx] ) {
        m_responses[idx]->reply(web::http::status_codes::OK, BJPutResponse(ST_PlaceBet, this->AsJSON()).AsJSON());
...
void									DealerTable::DealerDeal						()											{
    ...
    for (size_t i = 1; i < Players.size(); i++) {
        ...
        if ( player.Hand.bet	== 0.0			) { player.Hand.result								= HR_None;																continue; }
        if ( player.Hand.state	== HR_Busted	) { player.Hand.result								= HR_ComputerWin;														continue; }
        if ( player.Hand.state	== HR_BlackJack	) { player.Hand.result								= (dealersHand.state == HR_BlackJack) ? HR_Push : HR_PlayerBlackJack;	continue; }
        if ( dealersHand.state	== HR_Busted	) { player.Hand.result								= HR_PlayerWin;															continue; }
        if ( dealersHand.state	== HR_BlackJack ) { player.Hand.result								= HR_ComputerWin;														continue; }
        NumericHandValues										value							= player.Hand.GetNumericValues();
        player.Hand.result									= 
			(value.Best() <  handValue.Best()) ? HR_ComputerWin : 
...
#define ITER 4
    ...
    m_stopAt											= std::uniform_int_distribution<int>(26,52)(eng);
}
