...
struct NumericHandValues {
			...
			int									Best					()										const	noexcept	{ return (high < 22) ? high : low; }
};
...
struct Player {
	...
	inline										Player					()															{}
	inline										Player					(const utility::string_t &name)								: Name(name), Balance(1000.0) {}
...
struct BJTable {
    ...
    static BJTable								FromJSON				(const web::json::object &object)							{
        BJTable											result;
        web::json::object::const_iterator 
        objectIterator		= object.find(ID		);	if (objectIterator	== object.end()) throw web::json::json_exception(U("ID key not found"		));	result.Id		= (int)		objectIterator->second.as_double();
        objectIterator		= object.find(CAPACITY	);	if (objectIterator	== object.end()) throw web::json::json_exception(U("CAPACITY key not found"	));	result.Capacity	= (size_t)	objectIterator->second.as_double();
		objectIterator		= object.find(PLAYERS	);	if (objectIterator	== object.end()) throw web::json::json_exception(U("PLAYTERS key not found"	)); 
...
struct BJPutResponse {
	...
	inline										BJPutResponse						()												{}
	inline										BJPutResponse						(BJStatus status, web::json::value data)		: Status(status), Data(data)	{}

    static	BJPutResponse						FromJSON							(web::json::value object)						{ return BJPutResponse((BJStatus)(int)object[STATUS].as_double(), object[DATA]); }
			web::json::value					AsJSON								()							const				{
        web::json::value								result								= web::json::value::object();
