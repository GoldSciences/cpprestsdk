// HTTP Library: Oauth 2.0. For the latest on this and related APIs, please see: https://github.com/Microsoft/cpprestsdk
//
...
public:
    ...
    /// Returns access token validity state.
    ...
    /// Set refresh token.
    /// <param name="refresh_token">Refresh token string to set.
    ...
    /// <returns>Lifetime of the access token in seconds or undefined_expiration if not set.
    int64_t expires_in() const { return m_expires_in; }
    /// Set lifetime of access token (in seconds).
    /// <param name="expires_in">Lifetime of access token in seconds.
...
public:
    ...
    /// Returns authorization URI string.
    ...
    /// Returns task that fetches the token(s) based on redirected URI.
    ...
    /// <param name="authorization_code">Code received via redirect upon successful authorization.
    /// Returns task that fetches token(s) based on the authorization code.
    ...
    /// Returns task that fetches the token(s) using the refresh token.
    ...
    /// Set client key.
    /// <param name="client_key">Client key string to set.
    ...
    /// Set client secret.
    /// <param name="client_secret">Client secret string to set.
    ...
    /// <param name="redirect_uri">Redirect URI string to set.
    ...
    /// Returns scope string used in authorization.
    const utility::string_t& scope() const { return m_scope; }
    /// Set scope for authorization for token.
    /// <param name="scope">Scope string for authorization for token.
    void set_scope(utility::string_t scope) { m_scope = std::move(scope); }

    /// Get client state string used in authorization.
    /// <returns>Client state string used in authorization.
    ...
    /// <param name="state">Client authorization state string to set.
    ...
    /// <returns>Implicit grant setting for authorization.
    ...
    /// Default: True.
    /// <param name="bearer_auth">The bearer token authentication setting to set.
    void set_bearer_auth(bool bearer_auth) { m_bearer_auth = bearer_auth; }

    /// Get HTTP Basic authentication setting for token endpoint.
    /// <returns>HTTP Basic authentication setting for token endpoint.
    ...
    /// Default: True.
    /// <param name="http_basic_auth">The HTTP Basic authentication setting to set.
    void set_http_basic_auth(bool http_basic_auth) { m_http_basic_auth = http_basic_auth; }

    /// Get access token key.
    /// Returns access token key string.
