...
const oauth1_token& oauth1_config::token() const {
    if (m_is_authorization_completed)	// Return the token object only if authorization has been completed.
        return m_token;					// Otherwise the token object holds a temporary token which should not be returned to the user.
    else {
#if !defined(_WIN32) || _MSC_VER >= 1900
        static const oauth1_token empty_token;
#endif
