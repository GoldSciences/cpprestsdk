...
pplx::task<void> oauth2_config::_request_token(uri_builder& request_body_ub) {
    ...
    else {	// Add credentials to query as-is.
...
oauth2_token oauth2_config::_parse_token_from_json(const json::value& token_json) {
    ...
    else 												// Some services don't return 'token_type' while it's required by OAuth 2.0 spec: http://tools.ietf.org/html/rfc6749#section-5.1
        result.set_token_type(oauth2_strings::bearer);	// As workaround we act as if 'token_type=bearer' was received.
	...
	else {} // Do nothing. Preserves the old refresh token.
    ...
    if (token_json.has_field(oauth2_strings::scope))						// The authorization server may return different scope from the one requested.
        result.set_scope(token_json.at(oauth2_strings::scope).as_string());	// This however doesn't necessarily mean the token authorization scope is different.
																			// See: http://tools.ietf.org/html/rfc6749#section-3.3
    else	// Use the requested scope() if no scope parameter was returned.
