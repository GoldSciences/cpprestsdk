...
namespace {
    ...
    std::string convert_utf16be_to_utf8(utf16string src, bool erase_bom)			{ return utf16_to_utf8(big_endian_to_little_endian(std::move(src), erase_bom)); }
    utf16string convert_utf16be_to_utf16le(utf16string src, bool erase_bom)			{ return big_endian_to_little_endian(std::move(src), erase_bom); }
    ...
    std::string convert_utf16_to_utf8(utf16string src)
    {
        ...
        case unknown		: return convert_utf16be_to_utf8(std::move(src), false);	// unknown defaults to big endian.
        }
        __assume(0);
    }
    ...
    utf16string convert_utf16_to_utf16(utf16string src)
    {
        ...
        case unknown		: return convert_utf16be_to_utf16le(std::move(src), false);	// unknown defaults to big endian.
        }
        __assume(0);
    }
...
void http_headers::set_content_type(utility::string_t type) { m_headers[http::header_names::content_type] = std::move(type); }
...
void http_headers::set_cache_control(utility::string_t control) { add(http::header_names::cache_control, std::move(control)); }
...
void http_headers::set_date(const utility::datetime& date) { m_headers[http::header_names::date] = date.to_string(utility::datetime::RFC_1123); }
...
void http_headers::set_content_length(utility::size64_t length) { m_headers[http::header_names::content_length] = utility::conversions::details::to_string_t(length); }
...
void http_msg_base::_prepare_to_receive_data()
{
    ...
    if (!outstream()) {
        // The user did not specify an outstream. We will create one...
        concurrency::streams::producer_consumer_buffer<uint8_t> buf;
        set_outstream(buf.create_ostream(), true);

        // Since we are creating the streambuffer, set the input stream so that the user can retrieve the data.
        set_instream(buf.create_istream());
    }

    // If the user did specify an outstream we leave the instream as invalid. It is assumed that user either has a read head
    // to the out streambuffer or the data is streamed into a container or media (like file) that the user can read from...
}
...
utf16string details::http_msg_base::extract_utf16string(bool ignore_content_type)
{
    ...
    else if (utility::details::str_icmp(charset, charset_types::latin1))	// Latin1
    {
        std::string body;
        body.resize((std::string::size_type)buf_r.in_avail());
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size()).get(); // There is no risk of blocking.
    ...
    else if (utility::details::str_icmp(charset, charset_types::utf16))	// utf-16
    {
        utf16string body;
        body.resize(buf_r.in_avail() / sizeof(utf16string::value_type));
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size() * sizeof(utf16string::value_type)); // There is no risk of blocking.
    ...
    else if (utility::details::str_icmp(charset, charset_types::utf16be))	// utf-16be
    {
        utf16string body;
        body.resize(buf_r.in_avail() / sizeof(utf16string::value_type));
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size() * sizeof(utf16string::value_type)); // There is no risk of blocking.
...
utility::string_t details::http_msg_base::extract_string(bool ignore_content_type)
{
	...
	if(utility::details::str_icmp(charset, charset_types::latin1)) {		// Latin1
        std::string body;
        body.resize((std::string::size_type)buf_r.in_avail());
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size()).get(); // There is no risk of blocking.
    ...
    else if(utility::details::str_icmp(charset, charset_types::utf8)) {		// utf-8.
        std::string body;
        body.resize((std::string::size_type)buf_r.in_avail());
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size()).get(); // There is no risk of blocking.
        return to_string_t(std::move(body));
    ...
    else if(utility::details::str_icmp(charset, charset_types::utf16le)) {	// utf-16le
        utf16string body;
        body.resize(buf_r.in_avail() / sizeof(utf16string::value_type));
        buf_r.getn(const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(body.data())), body.size() * sizeof(utf16string::value_type)); // There is no risk of blocking.
...
{
    if (!instream)
        return utility::string_t();	// The instream is not yet set
...
utility::string_t details::http_msg_base::to_string() const { return http_headers_body_to_string(m_headers, instream()); }
