// HTTP Library: Client-side APIs. This file contains the implementation for the Windows Runtime, using XML HTTP Extended Request.
...
public:
    Microsoft::WRL::ComPtr<IXMLHTTPRequest2>					m_hRequest								= nullptr;
	std::exception_ptr											m_exceptionPtr							= {};
    // Request contexts must be created through factory function. But constructor needs to be public for make_shared to access.
																winrt_request_context					(const std::shared_ptr<_http_client_communicator> &client, http_request &request)					: request_context(client, request)									{}
    // Factory function to create requests on the heap.
    static std::shared_ptr<request_context>						create_request_context					(const std::shared_ptr<_http_client_communicator> &client, http_request &request)					{ return std::make_shared<winrt_request_context>(client, request);	}
};
...
public:
    ...
    HRESULT STDMETHODCALLTYPE									OnRedirect								(_In_opt_ IXMLHTTPRequest2*, __RPC__in_string const WCHAR*)											{ return S_OK; }	// Called when the HTTP request is being redirected to a new URL.
    HRESULT STDMETHODCALLTYPE									OnHeadersAvailable						(_In_ IXMLHTTPRequest2* xmlReq, DWORD dw, __RPC__in_string const WCHAR* phrase)						{					// Called when HTTP headers have been received and processed.
    ...
    HRESULT STDMETHODCALLTYPE									OnDataAvailable							(_In_opt_ IXMLHTTPRequest2*, _In_opt_ ISequentialStream* )											{ return S_OK; }	// Called when a portion of the entity body has been received.
    HRESULT STDMETHODCALLTYPE									OnResponseReceived						(_In_opt_ IXMLHTTPRequest2*, _In_opt_ ISequentialStream* )											{					// Called when the entire entity response has been received.
        auto															progress								= m_request->m_request._get_impl()->_progress_handler();
        ...
        //
        // Not releasing the winrt_request_context below previously worked due to the implementation of IXmlHttpRequest2, after calling OnError/OnResponseReceived it would immediately release its reference to HttpRequestCallback. 
		// However it since has been discovered on Xbox that the implementation is different, the reference to HttpRequestCallback is NOT immediately released and is only done at destruction of IXmlHttpRequest2.
    ...
    HRESULT STDMETHODCALLTYPE									OnError									(_In_opt_ IXMLHTTPRequest2*, HRESULT hrError)														{
        ...
        m_request.reset();	// Break the circular reference loop. See full explanation in OnResponseReceived.
        return S_OK;
    }
};

// This class acts as a bridge for the underlying request stream. These operations are completely synchronous, so it's important to block on both read and write operations. 
// The I/O will be done off the UI thread, so there is no risk of causing the UI to become unresponsive.
class IRequestStream
    : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<ClassicCom>, ISequentialStream>
{
    std::weak_ptr<winrt_request_context>						m_context;								// The request context controls the lifetime of this class so we only hold a weak_ptr.
    size_t														m_read_length;							// Length of the ISequentialStream for reads. This is equivalent to the amount of data that the ISequentialStream is allowed read from the underlying stream buffer.
public:
    // read_length is the initial length of the ISequentialStream that is available for read.
    // This is required because IXHR2 attempts to read more data that what is specified by the content_length. (Specifically, it appears to be reading 128K chunks regardless of the content_length specified).
																IRequestStream							(const std::weak_ptr<winrt_request_context> &context, size_t read_length = std::numeric_limits<size_t>::max())	: m_context(context), m_read_length	(read_length) {}

    virtual HRESULT STDMETHODCALLTYPE							Read									(_Out_writes_ (cb) void *pv, _In_ ULONG cb, _Out_ ULONG *pcbRead)									{
        auto															context									= m_context.lock();
        if(context == nullptr)
            return STG_E_READFAULT;	// OnError has already been called so just error out

		try {
            auto															buffer									= context->_get_readbuffer();
			msl::safeint3::SafeInt<size_t>									safe_count								= static_cast<size_t>(cb);	// Do not read more than the specified read_length
...
// This class acts as a bridge for the underlying response stream. These operations are completely synchronous, so it's important to block on both read and write operations. 
// The I/O will be done off the UI thread, so there is no risk of causing the UI to become unresponsive.
class IResponseStream : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<ClassicCom>, ISequentialStream>
{
    std::weak_ptr<winrt_request_context>						m_context;	// The request context controls the lifetime of this class so we only hold a weak_ptr.
public:
	...
	virtual HRESULT STDMETHODCALLTYPE							Write									(_In_reads_bytes_(cb) const void *pv, _In_ ULONG cb, _Out_opt_ ULONG *pcbWritten)					{
		auto															context									= m_context.lock();
		if(context == nullptr)
			return STG_E_CANTSAVE;	// OnError has already been called so just error out
...
public:
																winrt_client							(http::uri&& address, http_client_config&& client_config)											: _http_client_communicator(std::move(address), std::move(client_config))	{}
	...
	virtual	pplx::task<http_response>							propagate								(http_request request)																	override	{
		...
		auto															result_task								= pplx::create_task(context->m_request_completion);	// Use a task to externally signal the final result and completion of the task.
		this->async_send_request(context);	// Asynchronously send the response with the HTTP client implementation.
		return result_task;
	}
protected:
	unsigned long												open									()																									{ return 0; }	// Method to open client.
	void														send_request							(_In_ const std::shared_ptr<request_context> &request)												{				// Start sending request.
		...
		if (msg.method() == http::methods::TRCE) {
			request->report_exception(http_exception(L"TRACE is not supported"));	// Not supported by WinInet. Generate a more specific exception than what WinInet does.
			return;
		}
		const size_t													content_length							= msg._get_impl()->_get_content_length();
		if (content_length == std::numeric_limits<size_t>::max()) { // IXHR2 does not allow transfer encoding chunked. So the user is expected to set the content length
			request->report_exception(http_exception(L"Content length is not specified in the http headers"));
			return;
		}
		// Start sending HTTP request.
		HRESULT															hr										= CoCreateInstance(__uuidof(FreeThreadedXMLHTTP60), nullptr, CLSCTX_INPROC, __uuidof(IXMLHTTPRequest2), reinterpret_cast<void**>(winrt_context->m_hRequest.GetAddressOf()));
		if (FAILED(hr)) {
		...
		{	// New scope to ensure plain text password is cleared as soon as possible.
			...
			::web::details::plaintext_string								password_plaintext
		...
		// If XHR_PROP_ONDATA_NEVER is defined in ixmlhttprequest.h or msxml6.h utilize it. Specifies never to call OnDataAvailable improving performance and we already don't use OnDataAvaliable anyway.
...
#endif
		for (const auto &hdr : msg.headers())	// Add headers.
			winrt_context->m_hRequest->SetRequestHeader(hdr.first.c_str(), hdr.second.c_str());
		hr														= winrt_context->m_hRequest->SetCustomResponseStream(Make<IResponseStream>(winrt_context).Get());	// Set response stream.
		if (FAILED(hr)) {
			request->report_error(hr, L"Failure to set HTTP response stream");
			return;
		}
		try {	// Call the callback function of user customized options
