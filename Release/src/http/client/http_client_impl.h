...
public:
    ...
    void												report_exception				(const _ExceptionType &e)     { report_exception(std::make_exception_ptr(e)); }
    concurrency::streams::streambuf<uint8_t>			_get_writebuffer				();
    std::shared_ptr<_http_client_communicator>			m_http_client;					// Reference to the http_client implementation.
    ...
    pplx::task_completion_event<http_response>			m_request_completion;		// task completion event to signal request is completed.
    pplx::cancellation_token_registration				m_cancellationRegistration;	// Registration for cancellation notification if enabled.
};
