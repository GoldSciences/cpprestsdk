...
uri					details::_http_request::absolute_uri		()						const	{ return m_base_uri.is_empty() ? m_uri : uri_builder(m_base_uri).append(m_uri).to_uri(); }
...
utility::string_t	details::_http_response::to_string			()						const	{
    // If the user didn't explicitly set a reason phrase then we should have it default if they used one of the standard known status codes.
