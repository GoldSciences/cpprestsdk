...
static void query_header_length(HINTERNET request_handle, DWORD header, DWORD &length) {
    WinHttpQueryHeaders(request_handle, header, WINHTTP_HEADER_NAME_BY_INDEX, WINHTTP_NO_OUTPUT_BUFFER, &length, WINHTTP_NO_HEADER_INDEX);
}
...
static http::status_code parse_status_code(HINTERNET request_handle) {
    ...
    WinHttpQueryHeaders(request_handle, WINHTTP_QUERY_STATUS_CODE, WINHTTP_HEADER_NAME_BY_INDEX, &buffer[0], &length, WINHTTP_NO_HEADER_INDEX);
...
static DWORD ChooseAuthScheme( DWORD dwSupportedSchemes )
{
    ...
    //
		 if( dwSupportedSchemes & WINHTTP_AUTH_SCHEME_NEGOTIATE )	return WINHTTP_AUTH_SCHEME_NEGOTIATE;
    else if( dwSupportedSchemes & WINHTTP_AUTH_SCHEME_NTLM )		return WINHTTP_AUTH_SCHEME_NTLM;
    else if( dwSupportedSchemes & WINHTTP_AUTH_SCHEME_PASSPORT )	return WINHTTP_AUTH_SCHEME_PASSPORT;
    else if( dwSupportedSchemes & WINHTTP_AUTH_SCHEME_DIGEST )		return WINHTTP_AUTH_SCHEME_DIGEST;
    else if( dwSupportedSchemes & WINHTTP_AUTH_SCHEME_BASIC )		return WINHTTP_AUTH_SCHEME_BASIC;
    else
        return 0;
}
...
protected:
    ...
    unsigned long open()
    {
        ...
        // Open session.
        m_hSession = WinHttpOpen(NULL, access_type, proxy_name, WINHTTP_NO_PROXY_BYPASS, WINHTTP_FLAG_ASYNC);
        if(!m_hSession)
    ...
    void send_request(_In_ const std::shared_ptr<request_context> &request) {
        ...
        if(proxy_info_required) {
            auto result = WinHttpSetOption(winhttp_context->m_request_handle, WINHTTP_OPTION_PROXY, &info, sizeof(WINHTTP_PROXY_INFO) );
            if(!result) {
                auto errorCode = GetLastError();
        ...
        if (client_config().credentials().is_set()) {
            ...
            auto result = WinHttpSetOption(winhttp_context->m_request_handle, WINHTTP_OPTION_AUTOLOGON_POLICY, &data, sizeof(data));
            if(!result) {
                auto errorCode = GetLastError();
        ...
        if(!client_config().validate_certificates())
        {
            ...
            auto result = WinHttpSetOption(winhttp_context->m_request_handle, WINHTTP_OPTION_SECURITY_FLAGS, &data, sizeof(data));
            if(!result) {
                auto errorCode = GetLastError();
        ...
        if (content_length > 0) {
            ...
            if (content_length == std::numeric_limits<size_t>::max()) {
                // The content length is unknown and the application set a stream. This is an indication that we will use transfer encoding chunked.
        ...
        if(!msg.headers().empty()) {
            ...
            if(!WinHttpAddRequestHeaders(winhttp_context->m_request_handle, flattened_headers.c_str(), static_cast<DWORD>(flattened_headers.length()), WINHTTP_ADDREQ_FLAG_ADD)) {
                auto errorCode = GetLastError();
...
private:
        ...
        auto after_read = [p_request_context, chunk_size](pplx::task<size_t> op)
        {
            ...
            try {
                ...
                if (p_request_context->m_readBufferCopy) {
                    // We have raw memory here writing to a memory stream so it is safe to wait since it will always be non-blocking.
            ...
            if (!WinHttpWriteData(p_request_context->m_request_handle, &p_request_context->m_body_data.get()[offset], static_cast<DWORD>(length), nullptr)) {
                auto errorCode = GetLastError();
    ...
    static void _multiple_segment_write_data(_In_ winhttp_request_context * p_request_context)
    {
        ...
        {
            if (length == 0) {	// Unexpected end-of-stream.
        ...
        _In_ DWORD error = 0)
    {
        ...
        if (error != ERROR_WINHTTP_RESEND_REQUEST)
        {
            ...
            if(!WinHttpQueryAuthSchemes(hRequestHandle, &dwSupportedSchemes, &dwFirstScheme, &dwAuthTarget))
                return false;	// This will return the authentication failure to the user, without reporting fatal errors
        ...
        {
            // There is a request body that needs to be transferred.
            if (content_length == std::numeric_limits<size_t>::max())			// The content length is unknown and the application set a stream. This is an
            	p_request_context->m_bodyType = transfer_encoding_chunked;		// indication that we will need to chunk the data.
            else {
                // While we won't be transfer-encoding the data, we will write it in portions.
    ...
    static void CALLBACK completion_callback(HINTERNET hRequestHandle, DWORD_PTR context, DWORD statusCode, _In_ void* statusInfo, DWORD statusInfoLength)
    {
            ...
            case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE :
                {
                        ...
                        response.status_code() == status_codes::ProxyAuthRequired /*407*/)
                    {
                        ...
                        if(resending)
                            return;	// The request was not completed but resent with credentials. Wait until we get a new response
                    }
            ...
            case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE :
                {
                    ...
                    if(num_bytes > 0) {
                        if (p_request_context->decompressor)								// Decompression is too slow to reliably do on this callback. Therefore we need to store it now in order to decompress it at a later stage in the flow.
                            p_request_context->allocate_reply_space(nullptr, num_bytes);	// However, we want to eventually use the writebuf to store the decompressed body. Therefore we'll store the compressed body as an internal allocation in the request_context
                        else {
                        ...
                        if(!WinHttpReadData(hRequestHandle, p_request_context->m_body_data.get(), num_bytes, nullptr)) {
                            auto errorCode = GetLastError();
