...
{
    // Some things like proper URI schema are verified by the URI class. We only need to check certain things specific to HTTP.
    if(address.scheme() != U("http") && address.scheme() != U("https")) {	// HTTP Server API includes SSL support
...
http_listener::~http_listener() {
    if(m_impl) {				
        try {				// As a safe guard close the listener if not already done.	
            close().wait();	// Users are required to call close, but this is just a safeguard.
        } catch(...)
        {}
    }
}
...
pplx::task<void> details::http_listener_impl::open() {
    ...
    {
        try {	// If failed to open need to mark as closed.
            openOp.wait();
        }
        catch(...) {
            m_closed = true;
...
void details::http_listener_impl::handle_request(http_request msg)
{
    ...
    if(m_supported_methods.count(mtd))
        ...
        http_response response(status_codes::MethodNotAllowed);			// Method is not supported.
        response.headers().add(U("Allow"), get_supported_methods());	// Send back a list of supported methods to the client.
        msg.reply(response);
    }
}
