...
utility::string_t uri::encode_impl(const utility::string_t &raw, const std::function<bool(int)>& should_encode)
{
    ...
    for (auto iter = utf8raw.begin(); iter != utf8raw.end(); ++iter)
    {
        ...
        else	// ASCII don't need to be encoded, which should be same on both utf8 and utf16.
...
utility::string_t uri::encode_uri(const utility::string_t &raw, uri::components::component component) {
    ...
    switch(component) {
    case components::host		: return uri::encode_impl(raw, [](int ch) -> bool { return ch > 127; });	// No encoding of ASCII characters in host name (RFC 3986 3.2.2) 
    case components::user_info	: return uri::encode_impl(raw, [](int ch) -> bool { return !uri_parser::is_user_info_character	(ch) || ch == '%' || ch == '+'; });
    case components::path		: return uri::encode_impl(raw, [](int ch) -> bool { return !uri_parser::is_path_character		(ch) || ch == '%' || ch == '+'; });
    case components::query		: return uri::encode_impl(raw, [](int ch) -> bool { return !uri_parser::is_query_character		(ch) || ch == '%' || ch == '+'; });
    case components::fragment	: return uri::encode_impl(raw, [](int ch) -> bool { return !uri_parser::is_fragment_character	(ch) || ch == '%' || ch == '+'; });
    case components::full_uri	:
    default						: return uri::encode_impl(raw, [](int ch) -> bool { return !uri_parser::is_unreserved			(ch) && !uri_parser::is_reserved(ch); });
    };
}
...
static int hex_char_digit_to_decimal_char(int hex) {
    ...
    else if(hex >= 'A' && hex <= 'F') decimal	= 10 + (hex - 'A');
    else if(hex >= 'a' && hex <= 'f') decimal	= 10 + (hex - 'a');
    else
...
utility::string_t uri::decode(const utility::string_t &encoded) {
    ...
    for(auto iter = encoded.begin(); iter != encoded.end(); ++iter) {
        ...
        else	// encoded string has to be ASCII.
...
bool uri::operator == (const uri &other) const {
    ...
    // Each individual URI component must be decoded before performing comparison. TFS # 375865
		 if(this->is_empty	() && other.is_empty	())							result	= true;
    else if(this->is_empty	() || other.is_empty	())							result	= false;
    else if(this->scheme	() != other.scheme		())							result	= false;	// scheme is canonicalized to lowercase
    else if(uri::decode(this->user_info	())	!= uri::decode(other.user_info()))	result	= false;
    else if(uri::decode(this->host		())	!= uri::decode(other.host()))		result	= false;	// host is canonicalized to lowercase
    else if(this->port()					!= other.port())					result	= false;
    else if(uri::decode(this->path		())	!= uri::decode(other.path()))		result	= false;
    else if(uri::decode(this->query		())	!= uri::decode(other.query()))		result	= false;
    else if(uri::decode(this->fragment	())	!= uri::decode(other.fragment()))	result	= false;
