// Websocket library: Client-side APIs. This file contains a cross platform implementation based on WebSocket++.
//
...
public:
    ...
    ~wspp_callback_client()
    {
        ...
        case CLOSING:
            ...
            lock.unlock();
            try {	// This will do nothing in the already-connected case
                pplx::task<void>(m_connect_tce).get();
            }
            catch (...) {}
            try {
                close().wait();	// This will do nothing in the already-closing case
            }
            catch (...) {}
            break;
        }
        m_state = DESTROYED;	// At this point, there should be no more references to me.
    }
    ...
    pplx::task<void> connect_impl()
    {
        ...
        client.set_message_handler([this](websocketpp::connection_hdl, const websocketpp::config::asio_client::message_type::ptr &msg)
        {
            if (m_external_message_handler) {
                ...
                case websocketpp::frame::opcode::binary	: incoming_msg.m_msg_type = websocket_message_type::binary_message					; break;
                case websocketpp::frame::opcode::text	: incoming_msg.m_msg_type = websocket_message_type::text_message					; break;
                default									: incoming_msg.m_msg_type = static_cast<websocket_message_type>(msg->get_opcode())	; break;	// Unknown message type. Since both websocketpp and our code use the RFC codes, we'll just pass it on to the user.
                }
    ...
    pplx::task<void> send(websocket_outgoing_message &msg)
    {
...

        {
            if (++m_num_sends == 1) // No sends in progress
                ...
                send_msg(msg);	// Start sending the message
            else {	// Only actually have to take the lock if touching the queue.
    ...
    void send_msg(websocket_outgoing_message &msg)
    {
        ...
        if (length == SIZE_MAX) { 
            if (is_buf.has_size()) {	// This indicates we should determine the length automatically.
            ...
            else {	// The stream needs to be buffered.
        ...
        else											// Acquire succeeded, assign the acquired pointer to sp_allocated. Use an empty custom destructor
            sp_allocated.reset(ptr, [](uint8_t *) {});	// so that the data is not released when sp_allocated goes out of scope. The streambuf will manage its memory.

        read_task.then([this_client, msg, sp_allocated, length]()
        {
            std::lock_guard<std::mutex> lock(this_client->m_wspp_client_lock);
            if (this_client->m_state > CONNECTED) {	// The client has already been closed.
        ...
        }).then([this_client, msg, is_buf, acquired, sp_allocated, length](pplx::task<websocketpp::lib::error_code> previousTask) mutable
        {
            ...
            try {	// Catch exceptions from previous tasks, if any and convert it to websocket exception.
...
private:
        ...
        websocketpp::lib::error_code &ec)
    {
        ...
        switch (msg.m_msg_type)
        {
        case websocket_message_type::text_message	: client.send(this_client->m_con, sp_allocated.get(), length, websocketpp::frame::opcode::text	, ec);	break;
        case websocket_message_type::binary_message	: client.send(this_client->m_con, sp_allocated.get(), length, websocketpp::frame::opcode::binary, ec);	break;
		case websocket_message_type::pong			: client.pong(this_client->m_con, "", ec);																break;
        default:	// This case should have already been filtered above.
            std::abort();
        }
    }
    ...
    void			set_message_handler	(const std::function<void(const websocket_incoming_message&)>& handler)											{ m_external_message_handler	= handler; }
    void			set_close_handler	(const std::function<void(websocket_close_status, const utility::string_t&, const std::error_code&)>& handler)	{ m_external_close_handler		= handler; }
    ...
    std::mutex																						m_wspp_client_lock;	// Used to safe guard the wspp client.
    State																							m_state;
    std::unique_ptr<websocketpp_client_base>														m_client;
    std::mutex																						m_send_lock;	// Guards access to m_outgoing_msg_queue
    std::queue<websocket_outgoing_message>															m_outgoing_msg_queue;	// Queue to order the sends
    std::atomic<int>																				m_num_sends;	// Number of sends in progress and queued up.

    
    std::function<void(websocket_incoming_message)>													m_external_message_handler;	// External callback for handling received and close event
    std::function<void(websocket_close_status, const utility::string_t&, const std::error_code&)>	m_external_close_handler;

    // Used to track if any of the OpenSSL server certificate verifications failed. 
	// This can safely be tracked at the client level since connections only happen once for each client.
#if defined(__APPLE__) || (defined(ANDROID) || defined(__ANDROID__)) || defined(_WIN32)
