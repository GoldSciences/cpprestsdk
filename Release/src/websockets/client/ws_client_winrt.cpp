...
public:
        ...
        m_connected(false)
    {
        ...
        for (const auto & header : headers) 
            if (!utility::details::str_icmp(header.first, protocolHeader))	// Unfortunately the MessageWebSocket API throws a COMException if you try to set the 'Sec-WebSocket-Protocol' header here. It requires you to go through their API instead.
    ...
    pplx::task<void> send(websocket_outgoing_message &msg) {
        ...
        case websocket_message_type::binary_message : m_msg_websocket->Control->MessageType = SocketMessageType::Binary;	break;
        case websocket_message_type::text_message	: m_msg_websocket->Control->MessageType = SocketMessageType::Utf8;		break;
        default:
        ...
        if (msg_pending == false)	// No sends in progress
    ...
    pplx::task<void>			close					()																												{
        return close(websocket_close_status::normal, _XPLATSTR("Normal"));	// Send a close frame to the server
    }

    pplx::task<void>			close					(websocket_close_status status, const utility::string_t &strreason=_XPLATSTR(""))								{
        m_msg_websocket->Close(static_cast<unsigned short>(status), Platform::StringReference(strreason.c_str()));	// Send a close frame to the server
        return pplx::create_task(m_close_tce);	// Wait for the close response frame from the server.
    }
...
websocket_callback_client::websocket_callback_client()									: m_client(std::make_shared<details::winrt_callback_client>(websocket_client_config()))		{}
websocket_callback_client::websocket_callback_client(websocket_client_config config)	: m_client(std::make_shared<details::winrt_callback_client>(std::move(config)))				{}
