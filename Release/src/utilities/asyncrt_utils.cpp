...
// On Linux we are using boost error codes which have the exact same mapping and are equivalent with std::generic_category error codes.
const std::error_category & __cdecl linux_category() { return std::generic_category(); }
...
utf16string __cdecl conversions::utf8_to_utf16(const std::string &s) {
    ...
    for (auto src = s.begin(); src != s.end(); ++src)
    {
        ...
        else
        {
            ...
            else if ((*src & BIT6) == 0) { codePoint = *src & LOW_5BITS; numContBytes = 1; }	// 2 byte character, 0x80 to 0x7FF
            else if ((*src & BIT5) == 0) { codePoint = *src & LOW_4BITS; numContBytes = 2; }	// 3 byte character, 0x800 to 0xFFFF
            else if ((*src & BIT4) == 0) { codePoint = *src & LOW_3BITS; numContBytes = 3; }	// 4 byte character, 0x10000 to 0x10FFFF
            else {
            ...
            if (codePoint >= SURROGATE_PAIR_START) {												// In UTF-16 U+10000 to U+10FFFF are represented as two 16-bit code units, surrogate pairs.
                codePoint -= SURROGATE_PAIR_START;													//  - 0x10000 is subtracted from the code point
                dest.push_back(utf16string::value_type((codePoint >> 10) | H_SURROGATE_START));		//  - high surrogate is 0xD800 added to the top ten bits
                dest.push_back(utf16string::value_type((codePoint & 0x3FF) | L_SURROGATE_START));	//  - low surrogate is 0xDC00 added to the low ten bits
            }
            else 													// In UTF-16 U+0000 to U+D7FF and U+E000 to U+FFFF are represented exactly as the Unicode code point value.
                dest.push_back(utf16string::value_type(codePoint));	// U+D800 to U+DFFF are not valid characters, for simplicity we assume they are not present but will encode
            														// them if encountered.
        }
    }
    return dest;
...
utility::string_t datetime::to_string(date_format format) const
{
    ...
    else if (format == ISO_8601)
    {
        ...
        if (frac_sec > 0) {	// Append fractional second, which is a 7-digit value with no trailing zeros. This way, '1200' becomes '00012'
            char buf[9] = { 0 };
    ...
    if (format != RFC_1123 && frac_sec > 0) {
        // Append fractional second, which is a 7-digit value with no trailing zeros. This way, '1200' becomes '00012'
        char buf[9] = { 0 };
...
utility::string_t __cdecl timespan::seconds_to_xml_duration(utility::seconds durationSecs) {
    ...
    auto numMins	= numSecs	/ 60; if(numMins	> 0)	numSecs		= numSecs % 60;		// Find the number of minutes
    auto numHours	= numMins	/ 60; if(numHours	> 0)	numMins		= numMins % 60;		// Hours
    auto numDays	= numHours	/ 24; if(numDays	> 0)	numHours	= numHours % 24;	// Days
    ...
    if (numHours	> 0) oss << numHours	<< _XPLATSTR("H");
    if (numMins		> 0) oss << numMins		<< _XPLATSTR("M");
    if (numSecs		> 0) oss << numSecs		<< _XPLATSTR("S");
...
utility::seconds __cdecl timespan::xml_duration_to_seconds(const utility::string_t &timespanString) {
    ...
    while (c != eof) {
        ...
        if (c == L'S' || c == eof)	{	numSecs += val; break; }	// seconds
    }
