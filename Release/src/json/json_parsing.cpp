...
template <typename Token>
void																		SetErrorCode			(Token &tk, json_error jsonErrorCode)					{ tk.m_error = std::error_code(jsonErrorCode, json_error_category()); }
...
template <typename CharType>
typename JSON_Parser<CharType>::int_type JSON_StreamParser<CharType>::PeekCharacter() { return m_streambuf->sgetc(); }
...
{
#if defined(_WIN32)
    static int															print_llu	(char	* ptr, size_t n, uint64_t val64)		{ return _snprintf_s_l (ptr, n, _TRUNCATE,  "%I64u", utility::details::scoped_c_thread_locale::c_locale(), val64); }
    static int															print_llu	(wchar_t* ptr, size_t n, uint64_t val64)		{ return _snwprintf_s_l(ptr, n, _TRUNCATE, L"%I64u", utility::details::scoped_c_thread_locale::c_locale(), val64); }
    static double														anystod		(const char* str)								{ return _strtod_l(str, nullptr, utility::details::scoped_c_thread_locale::c_locale()); }
    static double														anystod		(const wchar_t* str)							{ return _wcstod_l(str, nullptr, utility::details::scoped_c_thread_locale::c_locale()); }
#else
    static int		__attribute__((__unused__))							print_llu	(char* ptr, size_t n, unsigned long long val64)	{ return snprintf(ptr, n, "%llu", val64); }
    static int		__attribute__((__unused__))							print_llu	(char* ptr, size_t n, unsigned long		 val64)	{ return snprintf(ptr, n, "%lu", val64); }
...
bool JSON_Parser<CharType>::CompleteNumberLiteral(CharType first, Token &token)
{
    ...
    if (first == '-') {
        minus_sign = true;
        first = static_cast<CharType>(NextCharacter());	// Safe to cast because the check after this if/else statement will cover EOF.
    }
    else
        minus_sign = false;
    ...
    while (ch != eof<CharType>()) {
        ...
        else // Not expected number character?
            break;
    };
...
bool JSON_Parser<CharType>::CompleteComment(Token &token) {
    ...
    if ( ch == '/' ) {	// Line comment -- look for a newline or EOF to terminate.
        ch = NextCharacter();
        while ( ch != eof<CharType>() && ch != '\n')
            ch = NextCharacter();
    }
    else {	// Block comment -- look for a terminating "*/" sequence.
        ch = NextCharacter();
        while ( true ) {
            if ( ch == eof<CharType>())
                return false;
...
bool JSON_StringParser<CharType>::CompleteComment(typename JSON_Parser<CharType>::Token &token) {
    ...
    // efficient in copying data from the input to the token: do a memcpy() rather than one character at a time.
...
void convert_append_unicode_code_unit(JSON_Parser<wchar_t	>::Token &token, utf16char value) { token.string_val.push_back(value); }
...
inline bool JSON_Parser<CharType>::handle_unescape_char(Token &token) {
    ...
    case 'u'	:
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            decoded |= (val << (4 * (3 - i)));	// Add the input char to the decoded number
        }
    ...
    while (ch != '"') {
        ...
        if (ch == '\\') {
            ...
            if (!JSON_StringParser<CharType>::handle_unescape_char(token))
                return false;
            start = m_position;	// Reset start position and continue.
        }
        else if (ch >= CharType(0x0) && ch < CharType(0x20))
            return false;
...
try_again:
    ...
    case 't': if (!CompleteKeywordTrue	(result))								SetErrorCode(result, json_error::malformed_literal);			break;
    case 'f': if (!CompleteKeywordFalse	(result))								SetErrorCode(result, json_error::malformed_literal);			break;
    case 'n': if (!CompleteKeywordNull	(result))								SetErrorCode(result, json_error::malformed_literal);			break;
    case '"': if (!CompleteStringLiteral(result))								SetErrorCode(result, json_error::malformed_string_literal);		break;
    case '-':
    case '0':
    case '1':
    case '2':
    ...
    case '7':
    case '8':
    case '9': if (!CompleteNumberLiteral(static_cast<CharType>(ch), result))	SetErrorCode(result, json_error::malformed_numeric_literal);	break;
    case '/': if (!CompleteComment		(result))								SetErrorCode(result, json_error::malformed_comment);			break;
        goto try_again;	// For now, we're ignoring comments.
    default:
...
std::unique_ptr<web::json::details::_Value> JSON_Parser<CharType>::_ParseObject(typename JSON_Parser<CharType>::Token &tkn) {
    ...
    if (tkn.kind != JSON_Parser<CharType>::Token::TKN_CloseBrace)
        while (true) {
			...
			if (tkn.kind != JSON_Parser<CharType>::Token::TKN_Colon)	// State 2: Looking for a colon.	
				goto done;
...
web::json::value web::json::value::parse(utility::istream_t &stream, std::error_code& error)	{ return _parse_stream(stream, error); }

#ifdef _WIN32
web::json::value web::json::value::parse(std::istream& stream)									{ return _parse_narrow_stream(stream); }
web::json::value web::json::value::parse(std::istream& stream, std::error_code& error)			{ return _parse_narrow_stream(stream, error); }
#endif
